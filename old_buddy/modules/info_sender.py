...
from old_buddy.modules.connect_api import ConnectAPI, States, PrinterInfo, EmitEvents, Event
...
class InfoSender:
    def __init__(self, serial: Serial, state_manager: StateManager, connect_api: ConnectAPI, ip_updater: IPUpdater):
        # , sd_card: SDCard):
        self.info_inserters: List[Callable[[PrinterInfo], PrinterInfo]] = [self.insert_type_and_version,
    ...
    def get_printer_info(self):
        ...
        for inserter in self.info_inserters:
            ...
            try:
                ...
                log.debug(f"Function {inserter.__name__} timed out waiting for serial.")
            except WriteIgnored:
                log.debug(f"Function {inserter.__name__} got ignored, serial rejected writing to it. "
    ...
    def insert_type_and_version(self, printer_info: PrinterInfo) -> PrinterInfo:
        match = self.serial.write_wait_response("M862.2 Q", PRINTER_TYPE_REGEX, timeout=PRINTER_INFO_TIMEOUT)
        if match is not None:
            code = int(match.groups()[0])
            try:
                printer_info.type, printer_info.version = PRINTER_TYPES[code]
            except KeyError:
                log.exception("The printer version has not been found in the list of printers")
        return printer_info

    def insert_firmware_version(self, printer_info: PrinterInfo) -> PrinterInfo:
        match = self.serial.write_wait_response("M115", FW_REGEX, timeout=PRINTER_INFO_TIMEOUT)
        if match is not None:
