...
class PrinterPolling:
    ...
    def _get_sheet_settings(self):
        ...
        matches = self.do_multimatch(
            "D3 Ax0D49 C88", D3_OUTPUT_REGEX, to_front=True)
    ...
    def get_active_sheet(self):
        ...
        matches = self.do_matchable(
            "D3 Ax0Da1 C1", D3_OUTPUT_REGEX, to_front=True)
    ...
    def _eeprom_little_endian_uint32(self, address):
        match = self.do_matchable(f"D3 Ax{address} C4",
                                  D3_OUTPUT_REGEX,
                                  to_front=True)
        str_data = match.group("data").replace(" ", "")
        data = bytes.fromhex(str_data)
    ...
    def _get_total_filament(self):
        ...
        total_filament = self._eeprom_little_endian_uint32("0FF1")
        return total_filament * 1000

    def _get_total_print_time(self):
        """Gets the total print time from the eeprom"""
        total_minutes = self._eeprom_little_endian_uint32("0FED")
