...
class WatchedValue(WatchedThing):
    """
    ...
    def __init__(self,
        ...
        self.write_function: Callable[[WatchedValue], None] = write_function
...
class WatchedGroup(WatchedThing):
    """
    A group of watched items.
    Aggregates the validity signals from its members
    """
    def __init__(self, items: Iterable[WatchedThing]):
        super().__init__()
...
class InfoUpdater:
    ...
    def invalidate(self, ambiguous_item: Union[WatchedThing, str]):
        """
        ...
        if isinstance(ambiguous_item, str):
            item = self.tracked_items[ambiguous_item]
        elif isinstance(ambiguous_item, WatchedGroup):
            for group_item in ambiguous_item:
                self.invalidate(group_item)
        elif isinstance(ambiguous_item, WatchedValue):
            item = ambiguous_item
    ...
    def set_value(self, ambiguous_item: Union[WatchedValue, str], value):
        """
    ...
    def schedule_invalidation(self,
                              ambiguous_item: Union[WatchedValue, str],
                              interval=None,
                              force=False):
        """
        ...
        if interval is None:
            raise ValueError(f"No interval specified for item {item.name} "
                             f"has no default and none has been provided!")

        log.debug("Scheduling invalidation of item %s for %ss in the future",
                  item.name, interval)
        item.invalidate_at = time() + interval
        self.invalidate_timers.put((item.invalidate_at, item))
        self.invalidate_queue_event.set()

    def cancel_scheduled_invalidation(self, ambiguous_item: Union[WatchedValue,
                                                                  str]):
        """
        ...
        log.debug("Cancelling scheduled invalidation of item %s ", item.name)
        item.invalidate_at = inf

    # -- Private --

    @staticmethod
    def _time_out(item: WatchedValue):
        """
    ...
    def _gather(self, item: WatchedValue):
        """
    ...
    def _set_value(self, item, value):
        ...
        if changed:
            log.debug("Item %s got a new value! old: %s new: %s", item.name,
                      item.value, value)
        item.value = value
        item.write_function(item)
        item.valid = True
        item.times_out_at = inf
    ...
    def _process_invalidations(self):
        ...
        while self.running:
            try:
                ...
                current_time = time()
                if item.invalidate_at > current_time:
    ...
    def _process_timeouts(self):
        ...
        while self.running:
            try:
                ...
                current_time = time()
                if item.times_out_at > current_time:
