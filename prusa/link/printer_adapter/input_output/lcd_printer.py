...
class LCDPrinter(metaclass=MCSingleton):
    ...
    def enqueue_no_sn(self):
        self.enqueue_message(f"ERR: Cannot get S/N")
        self.enqueue_message(f"Update printer FW")
        self.enqueue_message(f"Likely not fixed yet")
        self.enqueue_message(f"So set SN manually")
        self.enqueue_message(f"Edit the RPi config")
        self.enqueue_message(f"/home/pi/.config/Pru", duration=1.5)
        self.enqueue_message(f"me/pi/.config/Prusa-", duration=0.6)
        self.enqueue_message(f"pi/.config/Prusa-Lin", duration=0.6)
        self.enqueue_message(f".config/Prusa-Link/c", duration=0.6)
        self.enqueue_message(f"nfig/Prusa-Link/conf", duration=0.6)
        self.enqueue_message(f"g/Prusa-Link/config.", duration=0.6)
        self.enqueue_message(f"rusa-Link/config.yam", duration=0.6)
        self.enqueue_message(f"a-Link/config.yaml")
        self.enqueue_message(f"example config is")
        self.enqueue_message(f"in the same folder.")
        self.enqueue_message(f"Section [SN]")
        self.enqueue_message(f"Key SERIAL_NUMBER")
        self.enqueue_message(f"Copy SN section into")
        self.enqueue_message(f"your config and fill")
        self.enqueue_message(f"its value.")
        self.enqueue_message(f"ERR: Cannot get S/N")
